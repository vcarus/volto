...
| `initial_mtu` | bytes | `1200` | Size of the first QUIC packets — a *UDP payload* size, not an IP packet size. Range 1200..1452. **Below 1200 is an error** (RFC 9000 §14) rather than a silent round-up; above 1452 is an error too, because an Ethernet frame leaves 1452 bytes of payload over IPv6 (1472 over IPv4), quinn's own MTU discovery stops at 1452 for that reason, and quinn applies `initial_mtu` with no ceiling of its own — so a handshake sent in packets no path carries leaves the server unreachable with nothing to fall back to |
...
| `max_auth_failures` | integer | `5` | Authentication failures tolerated on one connection before it is dropped; `0` disables it. A request that authenticates clears the failures charged to **the user it authenticated as**, so they cannot add up over the life of a working connection — but a peer holding one valid credential cannot clear its guesses at a second user's password by interleaving a good request. The run is charged to the first failure that names a user, which need not be the first failure: a run that never named anybody — requests with no credentials field, the benign "the app forgot the header" case — is cleared by any success, but one credential-less request in front of a guess does not make the guess anybody's but the guessed user's |
