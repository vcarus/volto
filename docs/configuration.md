...
| Key | Type | Default | Meaning |
|---|---|---|---|
| `udp_session_timeout` | seconds | `180` | Idle timeout for a UDP session. RFC 9298 §3.1 says a proxy SHOULD NOT go below 120; volto warns if you do. Also bounds a half-closed TCP tunnel whose surviving direction makes no progress (see the architecture doc) |
