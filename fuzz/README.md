...
| `gate` | `gate` — the D106 SNI gate: the QUIC v1 long header, the Initial packet protection under the keys the datagram's own Destination Connection ID derives, the frame walk that assembles the offset-0 CRYPTO stream, and the ClientHello reader that pulls `server_name` out of it | any UDP datagram that arrives at the port, judged before quinn sees it and before a handshake has begun |

`gate` is the odd one out twice over. Its input is a whole datagram rather than a fragment of a stream, and its interesting inputs cannot be guessed: reaching the ClientHello reader through a datagram means spelling an Initial packet that authenticates under keys derived from its own Destination Connection ID, which a fuzzer will not do by luck. So it has both a seed corpus and a second mode that hands CRYPTO bytes to the TLS reader directly. The seeds — a name we answer to, a name we do not, a ClientHello with no `server_name`, one cut short, and a second-flight Initial keyed by a packet the gate never saw — are built and written by a test in `tests/it_gate_shapes.rs` that also asserts the verdict each one was built for, so `cargo test` at the root is what regenerates `fuzz/corpus/gate/` and a verdict that moves is a red test rather than a corpus that quietly stops covering anything. Its oracles beyond "no panic": a verdict is a function of the datagram's bytes and the configured list alone, every shape the module documentation promises to pass does pass, a refusal that names a host implies a datagram that carried an Initial packet at all, and changing the Destination Connection ID of a refused datagram turns the refusal into a pass — which is the v0.9.1 rule that only a packet the gate could open may be refused for what its ClientHello said.
...
Being its own cargo workspace is what cargo-fuzz wants, and it also means nothing from the root manifest reaches here on its own. Three things are therefore restated in `Cargo.toml` beside this file, and each has to move when the root moves: `edition = "2024"`, so a target cannot keep an idiom `src/` has left behind; `rust-version`, which is a resolver floor rather than a support claim (edition 2024 implies the MSRV-aware resolver, and with the field absent that resolver floors on whatever toolchain is installed, which is how the two lockfiles drift apart); and `[lints.rust] unsafe_code = "deny"`, the root's policy for every target in the package. None of the three is asserted, so they are on the reader, and the reason for each is written next to it. A fourth used to be here, the `[patch.crates-io]` pin on quinn-proto, which went when 0.11.18 shipped the fixes it carried (D60); `tests/it_release_assets.rs` now asserts that neither manifest patches that crate at all.

`fuzz/Cargo.lock` is committed, and the `fuzz` job in `.github/workflows/ci.yml` type-checks the targets on stable with `--locked`. The graph the fuzzers run against is the one that was reviewed, then, rather than whatever resolves on the morning of the run — which is what makes a green CI check and a green container run weeks later statements about the same thing. Refresh it with `cargo generate-lockfile --manifest-path fuzz/Cargo.toml`, which rewrites this workspace's lockfile and not the root's; a `cargo update` at the root leaves this file exactly where it was, so a refresh before a release runs in both directories.
...
Swap `frame` for any of the names in the table above; `cargo +nightly fuzz list` enumerates the nine. The corpus accumulates under `fuzz/corpus/<target>/` on the host and is deliberately not committed; crash inputs land in `fuzz/artifacts/<target>/`.
